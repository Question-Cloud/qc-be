...
class UserSubscriptionServiceTest : BehaviorSpec() {
    private val userSubscriptionReader = mockk<UserSubscriptionReader>()
    private val userSubscriptionService = UserSubscriptionService(userSubscriptionReader)
    ...
    init {
        Given("구독 여부 확인 요청이 주어졌을 때") {
            ...
            every { userSubscriptionReader.isSubscribed(userId, creatorId) } returns true
            ...
            When("구독 여부를 확인하면") {
                ...
                Then("구독 중임을 반환한다") {
                    ...
                    verify(exactly = 1) { userSubscriptionReader.isSubscribed(userId, creatorId) }
                }
            }
        }
        ...
        Given("사용자가 구독 리스트 조회 요청을 했을 때") {
            ...
            val userSubscriptionDetails = creatorQueryDatas.mapIndexed { index, creatorQueryData ->
            ...
            every {
                userSubscriptionReader.getUserSubscriptionDetails(
                    userId,
                    pagingInformation
                )
            } returns userSubscriptionDetails
            ...
            When("구독 리스트를 조회하면") {
                ...
                Then("구독 리스트가 반환된다") {
                    ...
                    verify(exactly = 1) { userSubscriptionReader.getUserSubscriptionDetails(userId, pagingInformation) }
                }
            }
        }
        ...
        Given("내 구독 수 조회 요청이 주어졌을 때") {
            ...
            every { userSubscriptionReader.countMySubscribe(userId) } returns 3
            ...
            When("내 구독 수를 조회하면") {
                ...
                Then("내 구독 수를 반환한다") {
                    ...
                    verify(exactly = 1) { userSubscriptionReader.countMySubscribe(userId) }
                }
            }
        }
    }
