...
class UserAccountServiceTest : BehaviorSpec() {
    ...
    init {
        Given("전화번호가 등록된 사용자가 있을 때") {
            ...
            every { userRepository.getUserByPhone(phoneNumber) } returns user
            ...
            When("전화번호로 계정을 찾으면") {
                ...
                Then("이메일을 찾을 수 있다") {
                    ...
                    verify(exactly = 1) { userRepository.getUserByPhone(phoneNumber) }
                }
            }
        }
        ...
        Given("존재하지 않는 전화번호일 때") {
            ...
            every { userRepository.getUserByPhone(nonExistentPhoneNumber) } throws CoreException(Error.NOT_FOUND)
        ...
        Given("이메일 계정 사용자가 있을 때") {
            ...
            every { emailVerificationProcessor.createEmailVerification(user.uid, email, EmailVerificationType.ChangePassword) } returns emailVerification
            ...
            When("비밀번호 복구 메일을 발송하면") {
                ...
                Then("이메일 인증 정보가 생성되고 메일이 발송된다") {
                    verify(exactly = 1) { userRepository.getUserByEmail(email) }
                    verify(exactly = 1) { emailVerificationProcessor.createEmailVerification(user.uid, email, EmailVerificationType.ChangePassword) }
        ...
        Given("존재하지 않는 이메일일 때") {
            ...
            every { userRepository.getUserByEmail(nonExistentEmail) } throws CoreException(Error.NOT_FOUND)
        ...
        Given("유효한 비밀번호 변경 토큰이 있을 때") {
            ...
            every { emailVerificationProcessor.verifyEmailVerification(emailVerification.token, EmailVerificationType.ChangePassword) } returns emailVerification
            ...
            When("토큰으로 비밀번호를 변경하면") {
                ...
                Then("비밀번호가 변경된다") {
                    verify(exactly = 1) { emailVerificationProcessor.verifyEmailVerification(emailVerification.token, EmailVerificationType.ChangePassword) }
        ...
        Given("잘못된 토큰일 때") {
            ...
            every { emailVerificationProcessor.verifyEmailVerification(invalidToken, EmailVerificationType.ChangePassword) } throws CoreException(Error.UNAUTHORIZED_TOKEN)
