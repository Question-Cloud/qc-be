...
Spring Boot, Maria DB, JPA, Querydsl, Redis, Rabbit MQ, Mongo DB, Docker, AWS EC2, AWS RDS, AWS Elasticache
...
멀티 모듈 구성의 가장 큰 의도는 **재사용성**입니다. 

단일 모듈 방식으로 구성할 경우, 하나의 모듈에 과도한 책임과 의존성이 집중되어 프로젝트가 복잡해질 수 있습니다.

개발 예정인 Admin 백오피스 애플리케이션에서는 기존의 유저 애플리케이션에서 사용되는 

**도메인 클래스**, **도메인 로직**, **엔티티 클래스**, **레포지토리** 등을 동일하게 사용할 예정입니다. 

만약 `qc-core` 모듈을 별도로 분리하지 않았다면, 동일한 코드를 복사 붙여넣기 방식으로 관리했을 것입니다.

또한, `qc-external-pg-api`와 `qc-lock-manager` 같은 모듈들도 Admin 백오피스 애플리케이션에서 재사용될 예정입니다. 

이러한 모듈화를 통해 **중복 코드를 줄이고 유지보수를 용이하게** 만들 수 있습니다.
...
데이터 증가에 따른 성능 저하 발생함을 확인하였고 개선하기 위해 별도의 리뷰 평점 통계 데이터를 보관하는 테이블을 설계하여 해결하였습니다.
