...
class PostCommentServiceTest(
    ...
    fun `댓글 권한이 없는 사용자는 댓글을 작성할 수 없다`() {
        //given
        val creatorUser = UserFixtureHelper.createEmailUser("creator@test.com", "password123", UserStatus.Active, userRepository)
        val unauthorizedUser = UserFixtureHelper.createEmailUser("unauthorized@test.com", "password123", UserStatus.Active, userRepository)
    ...
    fun `댓글 작성자는 자신의 댓글을 수정할 수 있다`() {
        ...
        val comments = postCommentService.getPostComments(post.id, creatorUser.uid, pagingInformation)
    ...
    fun `댓글 권한이 없는 사용자는 댓글 목록을 조회할 수 없다`() {
        ...
        //when & then
        val exception = assertThrows<CoreException> {
            postCommentService.getPostComments(post.id, unauthorizedUser.uid, pagingInformation)
        }
        Assertions.assertThat(exception.error).isEqualTo(Error.FORBIDDEN)
    }
